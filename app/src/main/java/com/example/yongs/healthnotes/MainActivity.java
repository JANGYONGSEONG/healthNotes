...
public class MainActivity extends AppCompatActivity {
    ...
    private void switchToExercise(){
        ...
        ExerciseFragment mExerciseDivisionFragment = new ExerciseFragment();
        transaction.replace(R.id.fragment_container, mExerciseDivisionFragment, FRAGMENT_EXERCISE_TAG).commitAllowingStateLoss();
    }

    private void switchToMeal() {
        FragmentTransaction transaction = getFragmentManagerInstance().beginTransaction();
        MealPlanFragment mMealPlanFragment = new MealPlanFragment();
        transaction.replace(R.id.fragment_container, mMealPlanFragment, FRAGMENT_MEAL_TAG).commitAllowingStateLoss();

    }

    private void switchToBody() {
        FragmentTransaction transaction = getFragmentManagerInstance().beginTransaction();
        BodyCompositionFragment mBodyCompositionFragment = new BodyCompositionFragment();
        transaction.replace(R.id.fragment_container, mBodyCompositionFragment, FRAGMENT_BODY_TAG).commitAllowingStateLoss();

    }

    private void switchToStatics(){
        FragmentTransaction transaction = getFragmentManagerInstance().beginTransaction();
        StaticsFragment mStaticsFragment = new StaticsFragment();
        transaction.replace(R.id.fragment_container, mStaticsFragment, FRAGMENT_STATIC_TAG).commitAllowingStateLoss();
    }
